...
namespace vloam {
    ...
    void LaserMapping::init (ros::NodeHandle* nh_) {
        ...
        count_receive = std::vector<int>(4, 0);

        subLaserCloudCornerLast = nh->subscribe<sensor_msgs::PointCloud2>("/laser_cloud_corner_last", 100, &LaserMapping::laserCloudCornerLastHandler, this);
        subLaserCloudSurfLast = nh->subscribe<sensor_msgs::PointCloud2>("/laser_cloud_surf_last", 100, &LaserMapping::laserCloudSurfLastHandler, this);
        subLaserOdometry = nh->subscribe<nav_msgs::Odometry>("/laser_odom_to_init", 100, &LaserMapping::laserOdometryHandler, this);
        subLaserCloudFullRes = nh->subscribe<sensor_msgs::PointCloud2>("/velodyne_cloud_3", 100, &LaserMapping::laserCloudFullResHandler, this);
    ...
    void LaserMapping::laserCloudCornerLastHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloudCornerLast2)
    {
        cornerLastBuf = laserCloudCornerLast2;
        ++count_receive[0];
    }

    void LaserMapping::laserCloudSurfLastHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloudSurfLast2)
    {
        surfLastBuf = laserCloudSurfLast2;
        ++count_receive[1];
    }

    void LaserMapping::laserCloudFullResHandler(const sensor_msgs::PointCloud2ConstPtr &laserCloudFullRes2)
    {
        fullResBuf = laserCloudFullRes2;
        ++count_receive[2];
    }
    ...
    void LaserMapping::solveMapping () {
        // TODO: make the skip frame (5) as global parameter
        if (count_receive[0] > 1 or count_receive[1] > 1 or count_receive[2] > 1 or count_receive[3] > 5) {
            ROS_ERROR("Mapping receives wrong inputs, received message counter state is %d %d %d %d", count_receive[0], count_receive[1], count_receive[2], count_receive[3]);
            ROS_BREAK();
            return;
        }

        while (count_receive[0] == 1 && count_receive[1] == 1 && count_receive[2] == 1 && count_receive[3] == 5)
        {
            this->reset();
