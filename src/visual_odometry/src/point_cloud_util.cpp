...
namespace vloam {
    void PointCloudUtil::loadTransformations(const std::string& calib_cam_to_cam_file_path, const std::string& calib_velo_to_cam_file_path) {
        std::string line;
        std::string delim = " ";
        float value;
        std::string curr;
    ...
    void PointCloudUtil::visualizePointCloud(const std::string image_file_path, const bool select_downsampled) {
        ...
        for (i = 0; i < point_cloud_2d_.rows(); ++i) {
            ...
            if (ratio < 0.5) {
                cv::circle(
                    image_with_point_cloud, 
                    cv::Point(point_cloud_2d_(i, 0), point_cloud_2d_(i, 1)), // x, y
                    1,
                ...
                cv::circle(
                    image_with_point_cloud, 
                    cv::Point(point_cloud_2d_(i, 0), point_cloud_2d_(i, 1)),
                    1,
